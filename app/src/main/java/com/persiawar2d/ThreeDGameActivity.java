...
public final class ThreeDGameActivity extends Activity {
    private AdvancedThreeDRendererV2 renderer;
    private TextView hud;
    @Override public void onCreate(Bundle state){
        super.onCreate(state);getWindow().setFlags(1024,1024);
        FrameLayout root=new FrameLayout(this);
        renderer=new AdvancedThreeDRendererV2(this);
        ...
        hud=new TextView(this);hud.setTextColor(Color.WHITE);hud.setTextSize(16);hud.setGravity(Gravity.CENTER);hud.setPadding(18,12,18,12);FrameLayout.LayoutParams hp=new FrameLayout.LayoutParams(-2,-2,Gravity.TOP|Gravity.START);root.addView(hud,hp);setContentView(root);
        hud.post(new Runnable(){public void run(){hud.setText("HP "+renderer.getHp()+"   •   AMMO "+renderer.getAmmo()+"   •   KILLS "+renderer.getKills()+"\nLEFT: MOVE     RIGHT: AIM / FIRE");hud.postDelayed(this,200);}});
    }
