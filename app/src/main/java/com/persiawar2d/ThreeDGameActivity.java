...
public final class ThreeDGameActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){
        ...
        hud.setPadding(18,12,18,12);
        FrameLayout.LayoutParams hp=new FrameLayout.LayoutParams(-2,-2,Gravity.TOP|Gravity.START);
        root.addView(hud,hp);

        BattleMiniMapView map=new BattleMiniMapView(this);
        map.setClickable(false);
        root.addView(map,new FrameLayout.LayoutParams(-1,-1));

        BattleStoryView story=new BattleStoryView(this);
        story.setClickable(false);
        root.addView(story,new FrameLayout.LayoutParams(-1,-1));
