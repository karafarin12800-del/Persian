...
public class MainActivity extends Activity {
    ...
    public static class GameView extends View {
        ...
        final Paint p=new Paint(Paint.ANTI_ALIAS_FLAG);final Random random=new Random(7);final ArrayList<Enemy> enemies=new ArrayList<>();final ArrayList<Bullet> bullets=new ArrayList<>();
        float px,py,aimX,aimY,joyX,joyY;int wave=1,score=0,ammo=12,reserve=100,maxHp=100,hp=100,shield=0,weapon=0;boolean gameOver=false;long lastShot=0,lastSpawn=0;float worldW=2400,worldH=1400;public String playerSkin="classic";
        ...
        float clamp(float v,float a,float b){return Math.max(a,Math.min(b,v));}float distance(float ax,float ay,float bx,float by){return (float)Math.hypot(ax-bx,ay-by);}
        public void setGameJoystick(float x,float y){joyX=x;joyY=y;}public void moveByJoystick(float dx,float dy){float d=Math.max(1,(float)Math.hypot(dx,dy));float speed=5.5f;px=clamp(px+dx/d*speed,50,worldW-50);py=clamp(py+dy/d*speed,HUD_H+30,worldH-50);}
        public void reload(){if(gameOver||weapon!=0||ammo>=12||reserve<=0)return;int n=Math.min(12-ammo,reserve);ammo+=n;reserve-=n;}public void toggleWeapon(){if(!gameOver)weapon=weapon==0?1:0;}
