...
public class MainActivity extends Activity {
    ...
    public static class GameView extends View {
        ...
        void resetGame(){
            px=WORLD_SIZE*.50f;py=WORLD_SIZE*.55f;aimX=px+900;aimY=py;
            moveNX=moveNY=0;joystickDown=false;joystickPointer=-1;fireDown=false;firePointer=-1;
        ...
        void spawnWave(){
            ...
            for(int i=0;i<n;i++){
                double a=random.nextDouble()*Math.PI*2;float d=650+random.nextFloat()*750;
                float x=clamp(px+(float)Math.cos(a)*d,140,WORLD_SIZE-140),y=clamp(py+(float)Math.sin(a)*d,HUD_H+140,WORLD_SIZE-140);
        ...
        void tick(long now,float dt){
            if(gameOver)return;
            if(joystickDown&&Math.hypot(moveNX,moveNY)>.05f){movePlayer(moveNX*360f*dt,moveNY*360f*dt);animatePlayer(now);}
            else if(!joystickDown){playerFrame=0;king.setState(playerDir,playerFrame);}
            if(fireDown)shoot();
            if(now-lastShot>850)autoAim();
            for(Enemy e:enemies)if(e.hp>0){
                float dx=px-e.x,dy=py-e.y,d=Math.max(1,(float)Math.hypot(dx,dy));float speed=e.type==3?105f:e.type==2?88f:72f;
            ...
            updateBullets(dt);enemies.removeIf(e->e.hp<=0);if(enemies.isEmpty()&&now-lastSpawn>700){wave++;spawnWave();}
        }
        ...
        void updateBullets(float dt){
            for(int i=bullets.size()-1;i>=0;i--){Bullet b=bullets.get(i);b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;
        ...
        void shoot(){
            if(gameOver)return;long now=System.currentTimeMillis();if(now-lastShot<150)return;if(ammo<=0){reload();return;}
            if(now-lastShot>850)autoAim();float dx=aimX-px,dy=aimY-py,d=Math.max(1,(float)Math.hypot(dx,dy));
            bullets.add(new Bullet(px+dx/d*60,py+dy/d*60,dx/d*1250,dy/d*1250,30,true,.75f));ammo--;lastShot=now;
        }
        void melee(){
            if(gameOver)return;long now=System.currentTimeMillis();if(now-lastMelee<320)return;lastMelee=now;
            float dx=aimX-px,dy=aimY-py,d=Math.max(1,(float)Math.hypot(dx,dy));
            for(Enemy e:enemies)if(e.hp>0){float ex=e.x-px,ey=e.y-py,ed=Math.max(1,(float)Math.hypot(ex,ey));float dot=(ex*dx+ey*dy)/(ed*d);if(ed<165&&dot>.05f){e.hp-=45;if(e.hp<=0)score+=e.type==3?40:10;}}
        }
        ...
        void drawPlayer(Canvas c,float s){
            ...
            king.setState(playerDir,playerFrame);king.setAlpha(255);int size=Math.max(126,Math.round(170*s)),h=size/2;king.setBounds(Math.round(x-h),Math.round(y-h),Math.round(x+h),Math.round(y+h));king.draw(c);
        ...
        void drawEnemy(Canvas c,Enemy e,float s){
            if(enemyArt==null)return;float x=e.x*s,y=e.y*s;int size=Math.round((e.type==3?142:e.type==2?126:112)*s),h=size/2;enemyArt.setAlpha(255);enemyArt.setBounds(Math.round(x-h),Math.round(y-h),Math.round(x+h),Math.round(y+h));enemyArt.draw(c);
        ...
        void drawBullet(Canvas c,Bullet b,float s){
            float x=b.x*s,y=b.y*s;p.setStyle(Paint.Style.FILL);p.setColor(b.player?Color.rgb(255,220,95):Color.rgb(255,85,70));c.drawCircle(x,y,Math.max(4,6*s),p);
        }
        void drawHud(Canvas c){
            p.setStyle(Paint.Style.FILL);p.setColor(Color.argb(225,12,22,19));c.drawRect(0,0,getWidth(),HUD_H,p);p.setTextAlign(Paint.Align.LEFT);p.setTextSize(24);p.setColor(Color.rgb(245,205,105));c.drawText("PERSIA WAR 2.5D",22,32,p);
            p.setTextSize(17);p.setColor(Color.WHITE);c.drawText("WAVE "+wave+"   SCORE "+score,22,62,p);p.setTextAlign(Paint.Align.RIGHT);c.drawText("HP "+hp+"   AMMO "+ammo+"/"+reserve+(weapon==1?"   SWORD":"   FIREARM"),getWidth()-22,34,p);p.setTextAlign(Paint.Align.LEFT);
        }
        void drawControls(Canvas c){
            long age=System.currentTimeMillis()-joystickFadeAt;int alpha=joystickDown?235:(int)Math.max(18,235-age/3);p.setStyle(Paint.Style.FILL);
            p.setColor((alpha<<24)|0x59615B);c.drawCircle(joyBaseX,joyBaseY,104,p);p.setColor(((Math.min(255,alpha+20))<<24)|0xE0C875);c.drawCircle(joyX,joyY,42,p);
            p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(3);p.setColor(((Math.min(255,alpha+25))<<24)|0xF0D990);c.drawCircle(joyBaseX,joyBaseY,104,p);p.setStyle(Paint.Style.FILL);
            float br=Math.max(94,Math.min(132,getHeight()*.14f)),fireX=getWidth()*.83f,fireY=getHeight()*.76f,reloadX=getWidth()*.69f,reloadY=getHeight()*.89f,switchX=getWidth()*.91f,switchY=getHeight()*.89f,swordX=getWidth()*.69f,swordY=getHeight()*.60f;
            circleButton(c,fireX,fireY,br*1.08f,0xD09A4638);circleButton(c,reloadX,reloadY,br*.58f,0xB0446F5B);circleButton(c,switchX,switchY,br*.58f,0xB0446F5B);circleButton(c,swordX,swordY,br*.66f,0xB05D5140);
            textCenter(c,"FIRE",fireX,fireY+8,Color.WHITE,22);textCenter(c,"R",reloadX,reloadY+7,Color.WHITE,18);textCenter(c,"SW",switchX,switchY+6,Color.WHITE,15);textCenter(c,"⚔",swordX,swordY+9,Color.WHITE,25);
        }
        ...
        @Override public boolean onTouchEvent(MotionEvent e){
            int action=e.getActionMasked();if(gameOver){if(action==MotionEvent.ACTION_DOWN)resetGame();return true;}
            if(action==MotionEvent.ACTION_DOWN||action==MotionEvent.ACTION_POINTER_DOWN){
                int idx=e.getActionIndex(),id=e.getPointerId(idx);float x=e.getX(idx),y=e.getY(idx);float br=Math.max(94,Math.min(132,getHeight()*.14f));
                float fireX=getWidth()*.83f,fireY=getHeight()*.76f,reloadX=getWidth()*.69f,reloadY=getHeight()*.89f,switchX=getWidth()*.91f,switchY=getHeight()*.89f,swordX=getWidth()*.69f,swordY=getHeight()*.60f;
                if(near(x,y,fireX,fireY,br*1.30f)){firePointer=id;fireDown=true;shoot();return true;}
                if(near(x,y,reloadX,reloadY,br*.80f)){reload();return true;}
                if(near(x,y,switchX,switchY,br*.80f)){toggleWeapon();return true;}
                if(near(x,y,swordX,swordY,br*.88f)){melee();return true;}
                if(x<getWidth()*.52f&&y>HUD_H){joystickPointer=id;joystickDown=true;joystickFadeAt=System.currentTimeMillis();joyBaseX=clamp(x,104,getWidth()*.46f);joyBaseY=clamp(y,HUD_H+104,getHeight()-104);joyX=joyBaseX;joyY=joyBaseY;moveNX=moveNY=0;return true;}
                if(y>HUD_H){setAimFromScreen(x,y);return true;}
            }
            if(action==MotionEvent.ACTION_MOVE){
                for(int i=0;i<e.getPointerCount();i++){int id=e.getPointerId(i);float x=e.getX(i),y=e.getY(i);
                    if(id==joystickPointer){float dx=x-joyBaseX,dy=y-joyBaseY,d=Math.max(1,(float)Math.hypot(dx,dy)),max=76;if(d>max){dx=dx/d*max;dy=dy/d*max;}joyX=joyBaseX+dx;joyY=joyBaseY+dy;moveNX=dx/max;moveNY=dy/max;updateDirection(moveNX,moveNY);}
                    else if(id!=firePointer&&y>HUD_H){setAimFromScreen(x,y);}
                }
                return true;
            }
            if(action==MotionEvent.ACTION_UP||action==MotionEvent.ACTION_POINTER_UP||action==MotionEvent.ACTION_CANCEL){
                int idx=e.getActionIndex(),id=e.getPointerId(idx);if(id==joystickPointer){joystickPointer=-1;joystickDown=false;joystickFadeAt=System.currentTimeMillis();joyX=joyBaseX;joyY=joyBaseY;moveNX=moveNY=0;}if(id==firePointer){firePointer=-1;fireDown=false;}return true;
            }
            return true;
        }
