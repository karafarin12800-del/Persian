...
/** Core Persia War gameplay view. Renderer is separated so the world art is actually drawn. */
public class MainActivity extends Activity {
    ...
    public static class GameView extends View {
        ...
        boolean gameOver=false,joyActive=false;
        long lastShot=0,lastSpawn=0;
        float worldW=WorldRenderer.WORLD_SIZE,worldH=WorldRenderer.WORLD_SIZE;
        public String playerSkin="classic";

        public GameView(Context c){
            super(c); world=new WorldRenderer(c); p.setTypeface(Typeface.DEFAULT);
            resetGame(); setFocusable(true); setLayerType(View.LAYER_TYPE_HARDWARE,null);
        }
        void resetGame(){
            px=worldW*.5f;py=worldH*.55f;aimX=px+500;aimY=py;wave=1;score=0;ammo=12;reserve=100;hp=maxHp;shield=0;weapon=0;gameOver=false;enemies.clear();bullets.clear();spawnWave();invalidate();
        }
        ...
        void tick(){
            if(gameOver)return;long now=System.currentTimeMillis();
            if(now-lastSpawn>9000&&enemies.isEmpty()){wave++;spawnWave();}
            if(joyX!=0||joyY!=0)moveByJoystick(joyX,joyY);
        ...
        void drawWorld(Canvas c){
            float availableH=Math.max(1,getHeight()-HUD_H);float scale=Math.min(getWidth()/worldW,availableH/worldH);
            world.draw(c,px,py,scale,getWidth(),getHeight(),HUD_H);
        }
        void drawEntities(Canvas c){
            float scale=Math.min(getWidth()/worldW,(getHeight()-HUD_H)/worldH);float ox=getWidth()/2f-px*scale;float oy=(getHeight()+HUD_H)/2f-py*scale;
            c.save();c.translate(ox,oy);
            for(Bullet b:bullets)drawBullet(c,b,scale);
            for(Enemy e:enemies)drawEnemy(c,e,scale);
            drawPlayer(c,scale);
            c.restore();
        }
        void drawPlayer(Canvas c,float s){
            p.setStyle(Paint.Style.FILL);p.setColor(weapon==1?Color.rgb(218,172,64):Color.rgb(47,137,91));c.drawCircle(px*s,py*s,30*s,p);
            p.setColor(Color.rgb(225,196,143));c.drawCircle(px*s,(py-7)*s,13*s,p);
            p.setColor(Color.rgb(24,34,29));p.setStrokeWidth(Math.max(3,5*s));c.drawLine(px*s,py*s,aimX*s,aimY*s,p);
        }
        void drawEnemy(Canvas c,Enemy e,float s){p.setStyle(Paint.Style.FILL);p.setColor(e.type==3?Color.rgb(220,105,35):e.type==2?Color.rgb(180,45,55):Color.rgb(125,50,95));c.drawCircle(e.x*s,e.y*s,(e.type==3?34:27)*s,p);p.setColor(Color.rgb(25,25,25));c.drawCircle(e.x*s,(e.y-5)*s,7*s,p);}
        ...
        void drawHud(Canvas c){
            ...
            p.setColor(Color.rgb(245,205,105));p.setTextSize(25);c.drawText("PERSIA WAR 2.5D",22,32,p);
            p.setTextSize(18);p.setColor(Color.WHITE);c.drawText("WAVE "+wave+"   SCORE "+score,22,62,p);c.drawText("HP "+hp+"   AMMO "+ammo+"/"+reserve+(weapon==1?"   SWORD":"   FIREARM"),getWidth()-390,34,p);
            drawControls(c);
        ...
        void drawControls(Canvas c){
            float cx=getWidth()*.16f,cy=getHeight()*.79f,r=Math.min(getWidth(),getHeight())*.105f;
            p.setColor(Color.argb(80,255,255,255));c.drawCircle(cx,cy,r,p);p.setColor(Color.argb(125,245,205,105));float k=Math.min(r*.55f,(float)Math.hypot(joyX,joyY)*r*.12f);c.drawCircle(cx+clamp(joyX,-1,1)*k,cy+clamp(joyY,-1,1)*k,r*.45f,p);
            float bx=getWidth()*.84f,by=getHeight()*.79f,br=r*.72f;p.setColor(Color.argb(155,190,55,45));c.drawCircle(bx,by,br,p);p.setColor(Color.WHITE);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(Math.max(16,br*.55f));c.drawText(weapon==1?"⚔":"FIRE",bx,by+7,p);p.setTextAlign(Paint.Align.LEFT);
            p.setColor(Color.argb(125,40,80,60));c.drawCircle(getWidth()*.72f,getHeight()*.89f,r*.42f,p);c.drawCircle(getWidth()*.88f,getHeight()*.89f,r*.42f,p);p.setColor(Color.WHITE);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(15);c.drawText("R",getWidth()*.72f,getHeight()*.895f,p);c.drawText("SW",getWidth()*.88f,getHeight()*.895f,p);p.setTextAlign(Paint.Align.LEFT);
        }
        @Override public boolean onTouchEvent(MotionEvent e){
            float x=e.getX(),y=e.getY();
            if(e.getAction()==MotionEvent.ACTION_UP&&gameOver){resetGame();return true;}
            if(e.getAction()==MotionEvent.ACTION_DOWN||e.getAction()==MotionEvent.ACTION_MOVE){
                if(y>getHeight()*.65f&&x<getWidth()*.36f){joyActive=true;joyX=(x-getWidth()*.16f)/Math.max(1,getWidth()*.16f);joyY=(y-getHeight()*.79f)/Math.max(1,getHeight()*.79f);joyX=clamp(joyX,-1,1);joyY=clamp(joyY,-1,1);return true;}
                aimX=px+(x-getWidth()/2f)/Math.max(.0001f,getWidth()/worldW);aimY=py+(y-(getHeight()+HUD_H)/2f)/Math.max(.0001f,getWidth()/worldW);
                if(y>getHeight()*.66f&&x>getWidth()*.69f){shoot();return true;}
                if(y>getHeight()*.83f&&x>getWidth()*.66f&&x<getWidth()*.79f){reload();return true;}
                if(y>getHeight()*.83f&&x>getWidth()*.80f){toggleWeapon();return true;}
                return true;
            }
            if(e.getAction()==MotionEvent.ACTION_UP||e.getAction()==MotionEvent.ACTION_CANCEL){joyActive=false;joyX=joyY=0;return true;}
            return true;
        }
