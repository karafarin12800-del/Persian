    ...
    private void scanAssets(AssetManager am,String dir){
        try{
            ...
            for(String name:names){
                ...
                if(roads.size()<20&&contains(low,"road","street","asphalt","path","intersection","tile_road"))roads.add(b);
                else if(trees.size()<20&&contains(low,"tree","pine","palm","plant","vegetation"))trees.add(b);
                else if(grass.size()<20&&contains(low,"grass","bush","shrub","flower"))grass.add(b);
                else b.recycle();
            }
