...
/** Lightweight native Android 2.5D renderer for the square 6000x6000 world. */
public final class WorldRenderer {
    ...
    public WorldRenderer(Context context){
        background=context.getDrawable(R.drawable.achaemenid_background);
        ground=context.getDrawable(R.drawable.persia_ground);
        buildLayout();
    }
    ...
    private void buildLayout(){
        ...
        int i=0;
        for(int[] s:spots){
            buildings.add(new Building(s[0],s[1],s[2],s[3],i%6));
            if(i%3!=1)buildings.add(new Building(s[0]+s[2]+90,s[1]+35,210+(i%3)*55,190+(i%4)*30,(i+2)%6));
            i++;
        }
    ...
    public void draw(Canvas c,float playerX,float playerY,float cameraScale,float viewW,float viewH,float hudH){
        p.setStyle(Paint.Style.FILL);p.setAlpha(255);p.setColor(Color.rgb(38,58,47));c.drawRect(0,0,viewW,viewH,p);
    ...
    private void drawBackground(Canvas c,float s){
        // The previously loaded background was never drawn. It is now rendered as
        // one large, non-repeating world layer so the supplied background is visible.
        if(background==null)return;
        int w=Math.max(1,Math.round(WORLD_SIZE*s)),h=Math.max(1,Math.round(WORLD_SIZE*s));
        background.setAlpha(255);background.setBounds(0,0,w,h);background.draw(c);
    }

    private void drawGround(Canvas c,float s){
        p.setAlpha(70);p.setColor(Color.rgb(80,112,69));c.drawRect(0,0,WORLD_SIZE*s,WORLD_SIZE*s,p);
        if(ground!=null){
            int tile=Math.max(320,(int)(620*s));
            ground.setAlpha(82);
            for(int y=0;y<WORLD_SIZE*s;y+=tile)for(int x=0;x<WORLD_SIZE*s;x+=tile){ground.setBounds(x,y,x+tile,y+tile);ground.draw(c);}
            ground.setAlpha(255);
        }
        p.setAlpha(255);
    }
    ...
    private void drawRoads(Canvas c,float s){
        for(Road r:roads){
            p.setAlpha(210);p.setColor(Color.rgb(68,66,56));
            ...
            p.setAlpha(190);p.setColor(Color.rgb(198,184,145));p.setStrokeWidth(Math.max(2,r.w*.025f*s));
    ...
    private void drawBuilding(Canvas c,Building b,float px,float py,float s){
        float fade=blocked(b,px,py)?0.24f:1f;p.setAlpha((int)(255*fade));
        float x=b.x*s,y=b.y*s,w=b.w*s,lift=Math.max(18*s,b.h*.22f*s);
        ...
        p.setColor(walls[b.kind%walls.length]);c.drawRect(x,y-lift,x+w,y,p);
        p.setColor(Color.rgb(92,65,46));Path roof=new Path();roof.moveTo(x-10*s,y-lift);roof.lineTo(x+w*.5f,y-lift-34*s);roof.lineTo(x+w+10*s,y-lift);roof.close();c.drawPath(roof,p);
        p.setColor(Color.rgb(219,184,119));int cols=Math.max(2,(int)(b.w/95));for(int j=0;j<cols;j++){float wx=x+18*s+j*(w-36*s)/Math.max(1,cols-1);c.drawRect(wx,y-lift+30*s,wx+22*s,y-lift+70*s,p);}p.setColor(Color.rgb(72,52,39));c.drawRect(x+w*.42f,y-55*s,x+w*.58f,y,p);
        if(b.kind%2==0){p.setColor(Color.rgb(196,152,79));c.drawRect(x+w*.12f,y-lift-2*s,x+w*.36f,y-lift+10*s,p);}p.setAlpha(255);
    }

    private boolean blocked(Building b,float px,float py){
        float bx=b.x+b.w*.5f,by=b.y+b.h*.5f,camX=px,camY=py-700,dx=px-camX,dy=py-camY,len2=dx*dx+dy*dy;
        if(len2<1)return false;float t=((bx-camX)*dx+(by-camY)*dy)/len2;if(t<0||t>1)return false;float cx=camX+t*dx,cy=camY+t*dy;
        return cx>b.x-30&&cx<b.x+b.w+30&&cy>b.y-30&&cy<b.y+b.h+30&&Math.abs(cy-by)<b.h*.9f;
    }
    private void drawShrub(Canvas c,Shrub sh,float px,float py,float s){float fade=distance(sh.x,sh.y,px,py)<420&&blockedPoint(sh.x,sh.y,px,py)?.22f:1f;p.setAlpha((int)(255*fade));float x=sh.x*s,y=sh.y*s,r=sh.r*s;p.setColor(Color.rgb(49,94,53));c.drawCircle(x,y,r,p);p.setColor(Color.rgb(77,126,66));c.drawCircle(x-r*.35f,y-r*.25f,r*.65f,p);p.setAlpha(255);}
